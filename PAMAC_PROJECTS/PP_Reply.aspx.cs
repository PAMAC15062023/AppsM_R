...
{
    public partial class PP_Reply : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    if (Session["PHeader"] != null && Session["actnumber"] != null && Session["Subject"] != null)
                    {
                        string acthead = Session["PHeader"].ToString();
                        string actnumber = Session["actnumber"].ToString();
                        lblMsgXls.Text = "in reply";
                        lblHeader.Text = acthead + " Reply " + actnumber;
                        ...
                        dynamicRichTextBox.Text = "i AM A NEW RICH TEXT BOX";
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Session["actnumber"] != null && Session["PHeader"] != null && Session["UserName"] != null)
                {
                    string actnumber = Session["actnumber"].ToString();
                    string acthead = Session["PHeader"].ToString();
                    ...
                    if (newpath2 != "")
                    {
                        ...
                        FileUpload_2.SaveAs(excelPath2);
                    }
                    if (newpath3 != "")
                    {
                        excelPath3 = Server.MapPath("~/UploadedFiles/") + Path.GetFileName(FileUpload_3.PostedFile.FileName);
                        FileUpload_3.SaveAs(excelPath3);
                    }

                    string inputString = TxtSummary.Text;
                    ...
                    cmd.Parameters.AddWithValue("@Attachment_1", newpath1);
                    cmd.Parameters.AddWithValue("@Attachment_2", newpath2);
                    cmd.Parameters.AddWithValue("@Attachment_3", newpath3);
        ...
        protected void btnBold_Click(object sender, EventArgs e)
        {
            ...
            int txtlen = TxtSummary.Text.Length;
            //   String txt = TxtSummary.Text.Substring(0, 10);

            //if (TxtSummary.Font.Bold)
            //{
            //    TxtSummary.Font.Bold = false;
            //  //  TxtSummary.Text = txt ;
            ...
            //    TxtSummary.Font.Bold = true;


            //}

            ////lblMsgXls.Text = TxtSummary.Font.Name;
