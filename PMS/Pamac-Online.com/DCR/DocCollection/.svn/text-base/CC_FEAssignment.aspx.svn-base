<%@ Page Language="C#" MasterPageFile="~/DCR/DocCollection/CC_MasterPage.master" AutoEventWireup="true" CodeFile="~/DCR/DocCollection/CC_FEAssignment.aspx.cs" Inherits="CPV_CC_CC_FEAssignment" Theme="SkinFile" %>
 ...
 <table border="0" cellpadding="1" cellspacing="0" width="100%">   
    ...
    <tr><td align="right"><asp:Button ID="btnAssign1" runat="server" SkinId="btnAssingSkin" OnClick="btnAssign_Click" ValidationGroup="vgrpFE" /></td></tr>
     ...
     SelectCommand="Select [VERIFICATION_TYPE_ID], [VERIFICATION_TYPE_CODE] from VERIFICATION_TYPE_MASTER &#13;&#10;               WHERE VERIFICATION_TYPE_ID IN(20,21,22,23,24) "></asp:SqlDataSource>
        ...
        <asp:SqlDataSource ID="sdsFE" runat="server" ConnectionString="<%$ ConnectionStrings:CMConnectionString %>" ProviderName="<%$ ConnectionStrings:CMConnectionString.ProviderName %>" SelectCommand="SELECT DISTINCT FULLNAME, FE_ID FROM FE_VW_Dcr INNER JOIN FE_PINCODE_MAPPING ON EMP_ID=FE_ID where ([Centre_ID]=?) order by FULLNAME ">
        <SelectParameters>
            <asp:SessionParameter Name="CentreId" SessionField="CentreId" Type="String" />                            
        </SelectParameters> 
        </asp:SqlDataSource>
        ...
        <asp:SqlDataSource ID="sdsSearchFE" runat="server" ConnectionString="<%$ ConnectionStrings:CMConnectionString %>" ProviderName="<%$ ConnectionStrings:CMConnectionString.ProviderName %>" SelectCommand="SELECT DISTINCT FULLNAME, FE_ID FROM FE_VW_Dcr INNER JOIN FE_PINCODE_MAPPING ON EMP_ID=FE_ID where ([Centre_ID]=?) order by FULLNAME ">
         <SelectParameters>
            <asp:SessionParameter Name="CentreId" SessionField="CentreId" Type="String" />                            
        </SelectParameters> 
        </asp:SqlDataSource>
