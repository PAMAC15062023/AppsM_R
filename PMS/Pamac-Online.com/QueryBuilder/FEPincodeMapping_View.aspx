<%@ Page Language="C#" MasterPageFile="~/QueryBuilder/QueryBuilder.master" AutoEventWireup="true" CodeFile="FEPincodeMapping_View.aspx.cs" Inherits="Administrator_FEPincodeMapping_View" Theme="skinFile"%>
...
function Fun_Select1()
{
    var ArrSelected1 = new Array(); 
    var SelObj1 = document.getElementById("<%=lstFEName.ClientID%>");
    var hidEmp_id = document.getElementById("<%=hidEmp_id.ClientID%>");   
    
    var P;
    var count1 = 0;
    for (P=0; P<SelObj1.options.length; P++)
    {
        if (SelObj1.options[P].selected)
        {
        ArrSelected1[count1] = SelObj1.options[P].value; 
        count1++;
        }
    }
    
    hidEmp_id.value=ArrSelected1;    
...
<tr><td colspan="5">
        ...
        Client </td>
         <td>
                :</td>
                <td >
                    <asp:ListBox ID="lstClient" SelectionMode="Multiple" runat="server" DataSourceID="sdsClient" SkinID="lbSkin" DataTextField="CLIENT_NAME" DataValueField="CLIENT_ID" Width="200px"></asp:ListBox><asp:SqlDataSource ID="sdsClient" runat="server"  ProviderName="System.Data.OleDb"
    ...
    <tr>
        ...
        <td style="height: 14px"><asp:ListBox ID="lstFEName" SelectionMode="Multiple" runat="server" DataSourceID="sdsFeName" SkinID="lbSkin" DataTextField="FULLNAME" DataValueField="EMP_id" Width="200px">
        </asp:ListBox>
            <asp:SqlDataSource ID="sdsFeName" runat="server" ProviderName="System.Data.OleDb"
                SelectCommand="SELECT '0' as [Emp_id],'--All--' as [FULLNAME] UNION SELECT distinct  [EMP_ID], [FULLNAME] FROM [FE_VW] WHERE CENTRE_ID=? order by FULLNAME">
                <SelectParameters>
                    <asp:SessionParameter Name="?" SessionField="CENTREID" />
                </SelectParameters>
            </asp:SqlDataSource>
