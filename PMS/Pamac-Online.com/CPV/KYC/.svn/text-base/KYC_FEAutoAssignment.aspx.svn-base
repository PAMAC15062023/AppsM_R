<%@ Page Language="C#" MasterPageFile="~/CPV/KYC/KYC_MasterPage.master" AutoEventWireup="true" CodeFile="KYC_FEAutoAssignment.aspx.cs" Inherits="CPV_KYC_KYC_FEAutoAssignment" Theme="SkinFile"%>
...
<tr>
    <td colspan="6">
        <asp:SqlDataSource ID="sdsVeriType" runat="server" ConnectionString="<%$ ConnectionStrings:CMConnectionString %>"
            ProviderName="<%$ ConnectionStrings:CMConnectionString.ProviderName %>" SelectCommand="SELECT VERIFICATION_TYPE_ID, VERIFICATION_TYPE_CODE FROM VERIFICATION_TYPE_MASTER WHERE (ACTIVITY_ID = '1011')AND VERIFICATION_TYPE_ID IN('1','2','19') AND (PARENT_TYPE_CODE = 'VV')">
        </asp:SqlDataSource>
    </td>
</tr>
