...
<%--<asp:HiddenField ID="hidCaseId" Value='<%# Bind("Case_Id") %>' runat="server" />--%>
    ...
    <asp:ValidationSummary ID="ValidationSummary2" runat="server" DisplayMode="List"
     ...
     SelectCommand="SELECT [CASE_TYPE_ID], [case_type] FROM [CPV_CELLULAR_CASE_TYPE_MASTER]"></asp:SqlDataSource>
        ...
        <asp:SqlDataSource ID="sdsFE" runat="server" ConnectionString="<%$ ConnectionStrings:CMConnectionString %>" ProviderName="<%$ ConnectionStrings:CMConnectionString.ProviderName %>" SelectCommand="SELECT DISTINCT FULLNAME, FE_ID FROM FE_VW INNER JOIN FE_PINCODE_MAPPING ON EMP_ID=FE_ID order by FULLNAME "></asp:SqlDataSource>
