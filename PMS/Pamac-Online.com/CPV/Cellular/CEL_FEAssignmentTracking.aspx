<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="CEL_MasterPage.master" Theme="SkinFile" CodeFile="CEL_FEAssignmentTracking.aspx.cs" Inherits="CPV_Cellular_CEL_FEAssignmentTracking" %>
...
<fieldset><legend class="FormHeading">FE Assignment</legend>
 <table border="0" cellpadding="1" cellspacing="0" width="100%">   
    ...
    <tr>
        ...
        <td>
                <asp:DropDownList ID="ddlType" runat="server" DataSourceID="sdsVeriType" DataTextField="case_type" DataValueField="case_type_id" OnDataBound="ddlType_DataBound" AutoPostBack="false" SkinID="ddlSkin">
                </asp:DropDownList>
        </td>
        <td>
        </td>
...
<%--<asp:HiddenField ID="hidCaseId" Value='<%# Bind("Case_Id") %>' runat="server" />--%>
    </td></tr>
    ...
    <asp:ValidationSummary ID="ValidationSummary2" runat="server" DisplayMode="List"
     ...
     SelectCommand="SELECT [CASE_TYPE_ID], [case_type] FROM [CPV_CELLULAR_CASE_TYPE_MASTER]"></asp:SqlDataSource>
        <asp:SqlDataSource ID="sdsPincode" runat="server"  ProviderName="System.Data.OleDb" SelectCommand="SELECT [PINCODE] FROM [FE_PINCODE_MAPPING] ORDER BY [PINCODE]"></asp:SqlDataSource>
        <asp:SqlDataSource ID="sdsGV" runat="server"  ProviderName="System.Data.OleDb" DeleteCommand="delete from CPV_CC_FE_CASE_MAPPING where 1=2">
        </asp:SqlDataSource>
        <asp:SqlDataSource ID="sdsFE" runat="server"  ProviderName="System.Data.OleDb" SelectCommand="SELECT DISTINCT FULLNAME, FE_ID FROM FE_VW INNER JOIN FE_PINCODE_MAPPING ON EMP_ID=FE_ID order by FULLNAME "></asp:SqlDataSource>
        <asp:HiddenField ID="hdnVerificatioTypeID" runat="server" />
        <asp:SqlDataSource ID="sdsSearchFE" runat="server"  ProviderName="System.Data.OleDb" SelectCommand="SELECT DISTINCT FULLNAME, FE_ID FROM FE_VW INNER JOIN FE_PINCODE_MAPPING ON EMP_ID=FE_ID order by FULLNAME ">
        </asp:SqlDataSource>
