<%@ Page Language="C#" MasterPageFile="~/CPV/CC/CC_MasterPage.master" Theme ="SkinFile" AutoEventWireup="true" CodeFile="~/CPV/CC/CC_TC_Assign.aspx.cs" Inherits="CPV_CC_CC_TC_Assign" %>
...
<table border="0" cellpadding="0" cellspacing="0" style="width: 99%">
    ...
    <tr>
         ...
         <PagerSettings Position="Top" />
                ...
                <asp:SqlDataSource ID="sdsTC" runat="server" 
                    ProviderName="System.Data.OleDb" SelectCommand="SELECT DISTINCT Emp_id,FULLNAME FROM EMPLOYEE_MASTER WHERE (DESIGNATION_ID = '8') AND (DOL IS NULL OR  DOL=' ') AND (CENTRE_ID = ?) ORDER BY FULLNAME&#13;&#10;">
                    <SelectParameters>
   ...
   <tr>
    ...
    <tr><td align="right" style="width: 426px"><asp:Button ID="btnAssign1" runat="server" OnClick="btnAssign1_Click1" Font-Bold="True" Text="Assign Cases"  />  
     ...
     SelectCommand="Select [VERIFICATION_TYPE_ID], [VERIFICATION_TYPE_CODE] from VERIFICATION_TYPE_MASTER &#13;&#10;               WHERE VERIFICATION_TYPE_ID IN(3,4) AND (PARENT_TYPE_CODE = 'TV')"></asp:SqlDataSource>
       ...
       <%-- <asp:SqlDataSource ID="sdsSearchFE" runat="server"  ProviderName="System.Data.OleDb"SelectCommand="SELECT DISTINCT TC_ID,e.fullname AS 'TC Name','XXXXX'AS BT,'XXXXX' AS RT,'XXXXX' AS Total INTO #t FROM cpv_cc_tc_case_mapping a,cpv_cc_case_details b, employee_master e WHERE a.case_id=b.case_id AND a.tc_id=e.emp_id AND CONVERT(varchar(10),a.date_time,25) >= ? AND CONVERT(varchar(10),a.date_time,25) < ? AND b.client_id=(?) AND b.centre_id=(?) UPDATE c SET BT = (SELECT count(a.case_id) FROM cpv_cc_tc_case_mapping a, cpv_cc_case_details b WHERE a.case_id = b.case_id AND CONVERT(varchar(10),a.date_time,25) >= ? AND CONVERT(varchar(10),a.date_time,25) < ? AND b.client_id =(?) AND b.centre_id = (?) AND a.verification_type_id IN('3','4')) FROM #t c, cpv_cc_tc_case_mapping d WHERE c.tc_id = d.tc_id UPDATE c SET RT=(SELECT count(a.case_id) FROM cpv_cc_tc_case_mapping a,cpv_cc_case_details b WHERE a.case_id=b.case_id AND CONVERT(varchar(10),a.date_time,25) >= ? AND CONVERT(varchar(10),a.date_time,25) < ? AND b.client_id =(?) AND b.centre_id=(?) AND a.verification_type_id IN('3','4'))FROM #t c, cpv_cc_tc_case_mapping d WHERE c.tc_id=d.tc_id UPDATE #t SET Total=RT+BT SELECT * FROM #t">
         <SelectParameters>
             <asp:SessionParameter Name="CentreId" SessionField="centreId" />
